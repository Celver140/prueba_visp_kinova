...
"""
ibvs_cube_node.py
IBVS con descenso condicionado al error lateral.
"""
...
class IBVSCubeNode(Node):
    ...
    def __init__(self):
        ...
        self.last_center_error = float('inf')
        self.recovery_mode = False

        # Matriz corregida: Mapea frame óptico (Z avance, X derecha, Y abajo) 
        # al frame ROS estándar (X avance, Y izq, Z arriba)
        self.R_opt_to_kdl = np.array([
            [ 0.0,  0.0,  1.0],  # Z opt -> X kdl (Avance)
            [-1.0,  0.0,  0.0],  # X opt -> -Y kdl (Lateral)
            [ 0.0, -1.0,  0.0]   # Y opt -> -Z kdl (Vertical)
        ])
    ...
    def get_target_features(self, cv_image):
        ...
        mask = cv2.inRange(hsv, np.array([0, 70, 50]), np.array([10, 255, 255])) | cv2.inRange(hsv, np.array([170, 70, 50]), np.array([180, 255, 255]))
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours: return None, None, None, 0.0

        largest = max(contours, key=cv2.contourArea)
        area = cv2.contourArea(largest)
        if area < 50: return None, None, None, 0.0
        ...
        if corners is not None:
            ...
            if M["m00"] > 0: cx, cy = float(M["m10"] / M["m00"]), float(M["m01"] / M["m00"])
            else: return None, None, None, 0.0

        return cx, cy, corners, area
    ...
    def image_cb(self, msg: Image):
        ...
        cx, cy, corners, area = self.get_target_features(cv_image)

        # Rollback
        if cx is None:
            self._publish_qdot(-1.2 * (self.last_dq / dt), dt); return
        centroid_error_px = math.hypot(cx - self.cam.get_u0(), cy - self.cam.get_v0())
        if centroid_error_px > (self.last_center_error + 35.0) and not self.recovery_mode:
            self._publish_qdot(-1.0 * (self.last_dq / dt), dt); self.recovery_mode = True; return

        self.recovery_mode, self.last_center_error = False, centroid_error_px
        cv2.circle(cv_image, (int(cx), int(cy)), 6, (0, 165, 255), -1)

        corner_error_px = float('inf')
        if corners is not None:
            for i in range(4): cv2.circle(cv_image, (int(corners[i][0]), int(corners[i][1])), 5, (0, 255, 0), -1)
            corner_error_px = math.sqrt(np.mean((corners[:, 0] - np.array(self.u_star)) ** 2 + (corners[:, 1] - np.array(self.v_star)) ** 2))

        # Transiciones
        if self.state == 1 and centroid_error_px < self.centroid_error_threshold_px and corners is not None: self.state = 2
        elif self.state == 2 and (corners is None or centroid_error_px > self.recenter_error_threshold_px): self.state = 1
        elif self.state == 2 and corner_error_px < self.corner_error_threshold_px: self.state = 3
        elif self.state == 3 and (corners is None or centroid_error_px > self.recenter_error_threshold_px): self.state = 1
        elif self.state == 3 and corner_error_px > self.recenter_error_threshold_px: self.state = 2

        v_opt = np.zeros(6)
        if self.state == 1:
            ex, ey = (cx - self.cam.get_u0()) / self.cam.get_px(), (cy - self.cam.get_v0()) / self.cam.get_py()
            z_c = self._sample_depth(cx, cy) or self.fallback_depth_m
            
            # DESCENSO CONDICIONADO AL ERROR LATERAL
            # Si el error es mayor de 40px, baja la velocidad Z a cero para asegurar centrado previo
            descent_factor = max(0.0, 1.0 - (centroid_error_px / 60.0))
            ez = (z_c - self.Z_d) * descent_factor

            # Potencia el ajuste en X e Y para evitar pérdida del FOV
            v_opt[0] = self.lambda_gain * ex * z_c * 2.5
            v_opt[1] = self.lambda_gain * ey * z_c * 2.5
            v_opt[2] = self.lambda_gain * ez
        else:
            for i in range(4): self.s[i].buildFrom((corners[i][0] - self.cam.get_u0()) / self.cam.get_px(), (corners[i][1] - self.cam.get_v0()) / self.cam.get_py(), self._sample_depth(corners[i][0], corners[i][1]) or self.fallback_depth_m)
            v_opt = np.array([self.task.computeControlLaw()[i] for i in range(6)], dtype=float)
        ...
        v_base = R6_kdl_to_base @ (self.R6_opt_to_kdl @ v_opt)
        
        J = kdl_jacobian_to_np(jac_kdl)
        qdot = np.zeros(self.n_joints)

        if self.state in (1, 2): qdot = self._solve_qdot(J, v_base)
        elif self.state == 3:
            ...
            idx_free = [0, 1, 2, 5]
            qdot_free = self._solve_qdot(J[:, idx_free], v_base - J[:, 3:5] @ np.array([qdot[3], qdot[4]]))
